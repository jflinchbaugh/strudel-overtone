...
(deftest filter-env-param-test
  ...
  (testing "detune-env, pshift-env, fshift-env, pan-env, distort-env set pattern parameters correctly"
    (let [pat (-> (sut/s [:saw])
                  (sut/detune-env 0.4)
                  (sut/detune-perc 0.08)
                  (sut/pshift-env 12)
                  (sut/pshift-adsr 0.01 0.1 0.0 0.1)
                  (sut/fshift-env 100)
                  (sut/pan-env 0.5)
                  (sut/distort-env 0.8))
          ev (first (:events pat))
          params (:params ev)]
      (is (= 0.4 ((:detune-env params) 0 :detune-env)))
      (is (= :perc (:detune-env-type params)))
      (is (= 12 ((:pshift-env params) 0 :pshift-env)))
      (is (= :adsr (:pshift-env-type params)))
      (is (= 100 ((:fshift-env params) 0 :fshift-env)))
      (is (= 0.5 ((:pan-env params) 0 :pan-env)))
      (is (= 0.8 ((:distort-env params) 0 :distort-env))))))
