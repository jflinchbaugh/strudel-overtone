...
;; --- Randomness Re-exports ---

(import-vars rand
             global-seed seed! repeatable-rand irand srand choose wchoose choose-n rtake)

;; --- Signals Re-exports ---

(import-vars sig
             sig-range sine-sig saw-sig tri-sig square-sig cosine-sig adsr-sig)
...
(def rest-allowed-keys
  "Set of parameter keys that are allowed to attach even to rests."
  #{:monophonic :light-grid :pad-light :pad-color :pad})

(defn with-param
  "Updates pattern events with a specific parameter.
   Applies to active events, or to rests for non-audio parameters."
  [pattern key param-value]
  (update pattern :events
          (fn [evs]
            (map (fn [e]
                   (if (and (is-rest-params? (:params e))
                            (not (contains? rest-allowed-keys key)))
                     e
                     (let [v (wrap-number-fn param-value)
                           old-v (get-in e [:params key])]
                       (assoc-in e [:params key]
                                 (combine-param-values key old-v v)))))
                 evs))))
...
(defmacro def-env-helpers
  "Generates depth, ADSR, and Percussive pattern helper functions for a target envelope prefix."
  [prefix]
  (let [prefix-str (name prefix)
        env-fn-name   (symbol (str prefix-str "-env"))
        adsr-fn-name  (symbol (str prefix-str "-adsr"))
        perc-fn-name  (symbol (str prefix-str "-perc"))
        depth-key     (keyword (str prefix-str "-env"))
        type-key      (keyword (str prefix-str "-env-type"))
        att-key       (keyword (str prefix-str "-attack"))
        dec-key       (keyword (str prefix-str "-decay"))
        sus-level-key (keyword (str prefix-str "-s-level"))
        rel-key       (keyword (str prefix-str "-release"))]
    `(do
       (defn ~env-fn-name
         ~(str "Sets the " prefix-str " envelope modulation depth.")
         [~'pattern ~'depth]
         (set-param ~'pattern ~depth-key ~'depth))

       (defn ~adsr-fn-name
         ~(str "Sets all " prefix-str " ADSR envelope parameters at once.")
         [~'pattern ~'attack ~'decay ~'sustain-level ~'release]
         (params ~'pattern {~type-key :adsr
                            ~att-key ~'attack
                            ~dec-key ~'decay
                            ~sus-level-key ~'sustain-level
                            ~rel-key ~'release}))

       (defn ~perc-fn-name
         ~(str "Sets " prefix-str " percussive envelope parameters at once.")
         [~'pattern ~'attack-sec]
         (params ~'pattern {~type-key :perc
                            ~att-key ~'attack-sec})))))
...
(defn light-grid
  "Sets a grid lighting function on pattern events, creates a lighting pattern,
   or decorates an inline note/token.
